...
class simplecertificate {
    ...
    public function get_issue($user = null, $issueifempty = true) {
        ...
        if (!empty($this->issuecert) && $this->issuecert->userid == $userid) {
            ...
            if (has_capability('mod/simplecertificate:manage', $this->context, $userid)) {
                ...
                $event = \mod_simplecertificate\event\certificate_issued::create([
                    ...
                    'other' => [
                        ...
                        'certificatename' => $issuedcert->certificatename,
                    ],
                ]);
                $event->trigger();
    ...
    /**
     * Formats a string as CPF with middle masking (*.XXX.XXX-**)
     *
     * @param string $value The value to format (typically username)
     * @return string Formatted CPF string showing only middle digits or empty string if invalid
     */
    protected function format_cpf($value) {
        // Remove all non-numeric characters.
        $cpf = preg_replace('/[^0-9]/', '', $value);
        
        // Check if we have exactly 11 digits for a valid CPF.
        if (strlen($cpf) != 11) {
            // If not 11 digits, return the original value.
            return strip_tags($value);
        }
        
        // Format as ***.XXX.XXX-** (showing only middle 6 digits).
        return '***.' . 
               substr($cpf, 3, 3) . '.' . 
               substr($cpf, 6, 3) . '-**';
    }
    ...
    /**
     * Para a correção da data do certificado
     */
    protected function format_course_period($startdate, $enddate) {
        return date('d/m/Y', $startdate) . ' a ' . date('d/m/Y', $enddate);
    }
    ...
    protected function get_certificate_text($issuecert, $certtext = null) {
        ...
        /**
         * Período do curso: startdate = matrícula do usuário (fallback: início do curso)
         *                   enddate   = conclusão do usuário (fallback: data de emissão)
         */
        $courseid = $this->get_course()->id;

        // startdate: data de início da matrícula do usuário no curso.
        $enrolstart = $DB->get_record_sql(
            "SELECT MIN(ue.timestart) AS timestart
              ...
              WHERE e.courseid = :courseid
                ...
                AND ue.timestart > 0",
            ['courseid' => $courseid, 'userid' => $issuecert->userid]
        );

        if ($enrolstart && !empty($enrolstart->timestart)) {
            $periodstart = (int) $enrolstart->timestart;
        } else {
            // Fallback: data de início do curso.
            $course = $DB->get_record('course', ['id' => $courseid], 'startdate');
            $periodstart = (int) $course->startdate;
        }

        // enddate: data de conclusão do curso pelo usuário.
        $completion = $DB->get_record_sql(
            "SELECT MAX(c.timecompleted) AS timecompleted
               FROM {course_completions} c
              WHERE c.userid  = :userid
                AND c.course  = :courseid",
            ['userid' => $issuecert->userid, 'courseid' => $courseid]
        );

        if ($completion && !empty($completion->timecompleted)) {
            $periodend = (int) $completion->timecompleted;
        } else {
            // Fallback: data de emissão do certificado.
            $periodend = (int) $issuecert->timecreated;
        }

        $a->courseperiod = $this->format_course_period($periodstart, $periodend);

        // Datas separadas: inscrição (matrícula) e conclusão do aluno.
        $a->enroldate = $periodstart ? date('d/m/Y', $periodstart) : '';
        $a->completiondate = $periodend ? date('d/m/Y', $periodend) : '';
        ...
        // Format username as CPF (XXX.XXX.XXX-XX).
        $a->cpfmask2 = $this->format_cpf($user->username);
